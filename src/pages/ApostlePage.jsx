// src/pages/ApostlePage.jsx — 使徒+繁殖（中英双语 + tokenURI 成体图片）
...
const APO_ABI = [
  ...
  {type:'function',name:'attrs',inputs:[{name:'id',type:'uint256'}],outputs:[
    {name:'strength',type:'uint8'},{name:'element',type:'uint8'},{name:'gender',type:'uint8'},
    {name:'gen',type:'uint16'},{name:'genes',type:'uint256'},{name:'birthTime',type:'uint64'},
    {name:'cooldownEnd',type:'uint64'},{name:'motherId',type:'uint32'},{name:'fatherId',type:'uint32'}
...
function fmtTime(ts,lang){
  const t=Number(ts);if(t===0)return lang==='zh'?'无':'None'
...
function ElemIcon({i,size=14}){
  return <img src={ELEM_SVGS[i]} alt={ELEMS[i].key} style={{width:size,height:size,verticalAlign:'middle'}}/>
}

// ── 使徒图片：未成年=蛋图，成年=tokenURI（原版 Evolution Land 图片）
function ApostleImg({apo, size=80, style={}}){
  ...
  useEffect(()=>{
    if (!apo.isAdult) { setImgSrc(null); return }
    // 直接用 genes 拼接原版图片 URL，无需额外 RPC 调用
    if (apo.genes && apo.genes !== 0n) {
      setImgSrc(`https://api.evolution.land/apostle/${apo.genes.toString()}.png`)
      return
    }
    // fallback: 调用合约 tokenURI
    if (pc) {
      pc.readContract({
        ...
        args: [BigInt(apo.id)]
      }).then(uri => { if(uri) setImgSrc(uri) }).catch(()=>{})
    }
  }, [apo.id, apo.isAdult, apo.genes, pc])

  if (!apo.isAdult) {
    return <img src={APO_EGG_GIF} alt="egg"
      style={{width:size,height:size,opacity:.75,...style}}/>
  }

  const src = err || !imgSrc ? APO_EGG_GIF : imgSrc
  const elemFilter = err || !imgSrc
    ? `hue-rotate(${apo.element*72}deg) saturate(1.5)` : undefined

  return (
    <img src={src} alt={`apostle-${apo.id}`}
      onError={()=>setErr(true)}
      style={{width:size,height:size,filter:elemFilter,...style}}/>
  )
}

function ApostleCard({apo,isMe,onBreed,onDetail,lang}){
  const adult=apo.isAdult,coolLeft=timeLeft(apo.cooldownEnd),progress=apo.growth||0
  const gName=GENDER_NAME[lang][apo.gender]
  const elemName=lang==='zh'?ELEMS[apo.element].name:ELEMS[apo.element].nameEn
  return(
    <div className="ap-card" onClick={()=>onDetail&&onDetail(apo)}
      ...
      <div className="ap-card-img-wrap" style={{background:`linear-gradient(135deg,${ELEMS[apo.element].color}22,#0a0814)`}}>
        ...
        <div className="ap-card-badge" style={{background:adult?GENDER_COLOR[apo.gender]+'cc':'#60408088'}}>
          {adult?`${GENDER_ICON[apo.gender]}${gName}`:`🥚 ${progress}%`}
        </div>
      ...
      <div className="ap-card-body">
        ...
        <div style={{fontSize:'.7rem',color:ELEMS[apo.element].color}}>
          <ElemIcon i={apo.element} size={11}/>{elemName} · {lang==='zh'?'力量':'STR'}{apo.strength}
        </div>
        {isMe
          ?<div style={{fontSize:'.62rem',color:'#52c462',marginTop:2}}>⭐ {lang==='zh'?'我的':'Mine'}</div>
        ...
        {adult&&isMe&&!apo.inMining&&!coolLeft&&(
          <button className="ap-breed-btn" onClick={e=>{e.stopPropagation();onBreed(apo)}}>
            💕 {lang==='zh'?'繁殖':'Breed'}
          </button>
        )}
      </div>
...
function BreedModal({myApo,onClose,apostles,pc,address,wc,onDone,lang}){
  ...
  const isMale=myApo.gender===0,T=(zh,en)=>lang==='zh'?zh:en
  const pGender=T(isMale?'雌':'雄',isMale?'Female':'Male')
  const candidates=apostles.filter(a=>
    a.id!==myApo.id&&a.owner?.toLowerCase()===address?.toLowerCase()&&
    a.gender!==myApo.gender&&a.isAdult&&Number(a.cooldownEnd)*1000<Date.now()
  )
  useEffect(()=>{
    if(!pc)return
  ...
  async function doBreed(){
    ...
    try{
      if(breedFee>0n){
        setMsg(`${T('授权','Approve')} ${formatEther(breedFee)} RING...`)
    ...
    <div className="ap-modal-bg" onClick={onClose}>
      <div className="ap-modal" onClick={e=>e.stopPropagation()}>
        <div className="ap-modal-head">
          <span>💕 {T('繁殖','Breed')} — {GENDER_ICON[myApo.gender]}{T(GENDER_NAME.zh[myApo.gender],GENDER_NAME.en[myApo.gender])} #{myApo.id}</span>
          <button onClick={onClose}>✕</button>
        </div>
        {msg&&<div className="ap-modal-msg">{msg}</div>}
        {!partner?(<>
          <div style={{padding:'.6rem 1rem .3rem',fontSize:'.78rem',color:'#9070d0'}}>
            {T(`选择我的${pGender}使徒配对（已成年、无冷却）：`,`Select my ${pGender} apostle (adult, no cooldown):`)}
          </div>
          {candidates.length===0
            ?<div style={{padding:'1.5rem',textAlign:'center',color:'#5040a0'}}>
              {T(`暂无可配对的${pGender}使徒`,`No available ${pGender} apostle`)}<br/>
              <span style={{fontSize:'.72rem'}}>{T('需要已成年（孵化7天后）且无冷却期','Must be adult (7d after hatch) and no cooldown')}</span>
            </div>
            :<div className="ap-modal-grid">
              {candidates.map(a=>(<div key={a.id} className="ap-modal-item" onClick={()=>setPartner(a)}>
                <ApostleImg apo={a} size={38}/>
                <div style={{fontSize:'.72rem',color:'#c090ff'}}>#{a.id} G{a.gen}</div>
                <div style={{fontSize:'.65rem',color:ELEMS[a.element].color}}>
                  <ElemIcon i={a.element} size={11}/>
                  {lang==='zh'?ELEMS[a.element].name:ELEMS[a.element].nameEn} {T('力','STR')}{a.strength}
                </div>
              </div>))}
            </div>
          }
          ...
          <div style={{display:'flex',alignItems:'center',gap:'1.5rem',justifyContent:'center',marginBottom:'1rem'}}>
            <div style={{textAlign:'center'}}>
              <ApostleImg apo={myApo} size={56}/>
              <div style={{fontSize:'.75rem',color:'#c090ff'}}>#{myApo.id} {GENDER_ICON[myApo.gender]}</div>
              <div style={{fontSize:'.68rem',color:ELEMS[myApo.element].color}}>{T('力','STR')}{myApo.strength} G{myApo.gen}</div>
            </div>
            <div style={{fontSize:'1.5rem',color:'#c090ff'}}>💕</div>
            <div style={{textAlign:'center'}}>
              <ApostleImg apo={partner} size={56}/>
              <div style={{fontSize:'.75rem',color:'#c090ff'}}>#{partner.id} {GENDER_ICON[partner.gender]}</div>
              <div style={{fontSize:'.68rem',color:ELEMS[partner.element].color}}>{T('力','STR')}{partner.strength} G{partner.gen}</div>
            </div>
          </div>
...
export default function ApostlePage(){
  ...
  const [apostles,setApostles]=useState([]),[loading,setLoading]=useState(true)
  const [filter,setFilter]=useState('mine'),[breedApo,setBreedApo]=useState(null),[detailApo,setDetailApo]=useState(null)
      ...
      for(let s=1;s<=total;s+=BATCH){
        ...
        ids.forEach((id,i)=>{
          ...
          items.push({
            ...
            gen:Number(a[3]), genes:a[4], birthTime:a[5], cooldownEnd:a[6],
  ...
  const filterTabs=[
    ['mine',T('⭐ 我的','⭐ Mine')],['all',T('全部','All')],
    ['adult',T('已成体','Adult')],['egg',T('孵化中','Hatching')],
    ['male','♂ '+T('雄','Male')],['female','♀ '+T('雌','Female')],
  ]
    ...
    <div className="ap-root">
      ...
      <div className="ap-info-card">
        <div className="ap-info-row">
          ...
          <span>🧬 {T('后代继承基因','Offspring inherits genes')}</span>
        </div>
        <div style={{fontSize:'.72rem',color:'#4030a0',marginTop:6,textAlign:'center'}}>
          {T('成年后显示原版 Evolution Land 使徒图片（由基因决定外貌）','Adult apostles show Evolution Land original art based on genes')}
        </div>
      </div>
      ...
      {filter==='mine'&&!address&&(
        <div style={{textAlign:'center',color:'#5040a0',padding:'2rem',fontSize:'.85rem'}}>
          {T('请先连接钱包查看你的使徒','Please connect wallet to view your apostles')}
        </div>
      )}
      {loading?<div className="ap-loading"><span className="ap-spin"/>{T('加载中...','Loading...')}</div>
        :filtered.length===0
          ?<div className="ap-empty">
            {filter==='mine'&&address?T('你还没有使徒，去盲盒页购买吧','No apostles yet, buy from BlindBox'):T('暂无使徒','No apostles found')}
          </div>
          :<div className="ap-grid">
            {filtered.map(a=>(<ApostleCard key={a.id} apo={a} isMe={a.isMe} lang={lang}
      ...
      {detailApo&&(
        <div className="ap-modal-bg" onClick={()=>setDetailApo(null)}>
          <div className="ap-modal" onClick={e=>e.stopPropagation()}>
            <div className="ap-modal-head">
              <span>{T('使徒','Apostle')} #{detailApo.id}</span>
            ...
            <div style={{padding:'1rem',display:'flex',gap:'1rem'}}>
              ...
              <div style={{flex:1,display:'flex',flexDirection:'column',gap:6,fontSize:'.78rem',color:'#9080b0'}}>
                ...
                <div>
                  {GENDER_ICON[detailApo.gender]}
                  {T(GENDER_NAME.zh[detailApo.gender],GENDER_NAME.en[detailApo.gender])}
                  {' · '}<ElemIcon i={detailApo.element} size={12}/>
                  {lang==='zh'?ELEMS[detailApo.element].name:ELEMS[detailApo.element].nameEn}
                  {T('系 · 力量','· STR')}{detailApo.strength}
                </div>
                <div style={{color:detailApo.isAdult?'#52c462':'#c090ff'}}>
                  {detailApo.isAdult?T('✅ 已成年','✅ Adult'):`🥚 ${T('孵化中','Hatching')} ${detailApo.growth}%`}
                </div>
                <div>{T('持有者：','Owner: ')}
                  <span style={{fontFamily:'monospace',fontSize:'.72rem',color:detailApo.isMe?'#52c462':'#7060a0'}}>
                    {detailApo.isMe?`⭐ ${T('我的','Mine')}`:fmtAddr(detailApo.owner)}
                  </span>
                </div>
                <div>{T('生日：','Birthday: ')}{fmtTime(detailApo.birthTime,lang)}</div>
                {Number(detailApo.cooldownEnd)*1000>Date.now()&&(
                  <div style={{color:'#f0a040'}}>⏳ {T('繁殖冷却：','Breed CD: ')}{timeLeft(detailApo.cooldownEnd)}</div>
                )}
                ...
                {detailApo.genes&&<div style={{color:'#4030a0',fontSize:'.65rem'}}>{T('基因：','Genes: ')}#{detailApo.genes.toString().slice(0,20)}...</div>}
              </div>
            </div>
            {detailApo.isAdult&&detailApo.isMe&&Number(detailApo.cooldownEnd)*1000<Date.now()&&(
              <div style={{padding:'0 1rem 1rem'}}>
                <button className="ap-btn-primary" style={{width:'100%'}}
                  onClick={()=>{setDetailApo(null);setBreedApo(detailApo)}}>
                  💕 {T('去繁殖','Go Breed')}
                </button>
              </div>
            )}
      ...
      {breedApo&&<BreedModal myApo={breedApo} apostles={apostles} lang={lang}
